...
#
# 使用：
#   ./scripts/release.sh                     # 全交互，默认同时构建 amd64 + arm64
#   ./scripts/release.sh -v 1.3.0 -y         # 指定版本 + 跳过确认（CI 常用）
#   ./scripts/release.sh -v 1.3.0 --amd64-only       # 只发 amd64
#   ./scripts/release.sh -v 1.3.0 --arm64-only       # 只发 arm64
